...
<?else:?>
	<input type="text" class="search-input" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>"  />
<?endif;?>
	&nbsp;<input class="search-input-submit" type="submit" value="<?=GetMessage("SEARCH_GO")?>" />
...
<?elseif(count($arResult["SEARCH"])>0):?>
	...
	<?foreach($arResult["SEARCH"] as $arItem):?>
		<h4 style="font-weight:500;"><a href="<?echo $arItem["URL"]?>"><?echo $arItem["TITLE_FORMATED"]?></a></h4>
		...
		<?endif;?>
		<small style="margin-right: 20px;font-weight:600"><?=GetMessage("SEARCH_MODIFIED")?> <?=$arItem["DATE_CHANGE"]?></small><?
		if($arItem["CHAIN_PATH"]):?>
			<small style="font-weight:600"><?=GetMessage("SEARCH_PATH")?>&nbsp;<?=$arItem["CHAIN_PATH"]?></small><?
		endif;
		?><hr style="margin:10px 0px 22px 0px;border-top: 1px solid #f5f5f5;"/>
	<?endforeach;?>
