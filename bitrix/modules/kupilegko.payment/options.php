<?php
	...
	$moduleID = $ALFABANK_CONFIG['MODULE_ID'];
...
<?	
	...
	$current_settings = array(
		...
		'RBS_PROD_URL_AB' => COption::GetOptionString($moduleID, 'RBS_PROD_URL_AB'),
		'RBS_TEST_URL_AB' => COption::GetOptionString($moduleID, 'RBS_TEST_URL_AB'),
...
<?
	$tabControl = new CAdminTabControl("tabControl",  array(
		array("DIV" => "edit1", "TAB" => Loc::getMessage('ALFABANK_PAYMENT_TAB_NAME'), "ICON" => "blog_settings", "TITLE" => Loc::getMessage('ALFABANK_PAYMENT_TAB_TITLE')),
	));
	$tabControl->Begin();
?>
...
<form method="POST" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsbx($mid)?>&lang=<?=LANGUAGE_ID?>">
	...
	<? $tabControl->BeginNextTab(); ?>
		...
		<tr class="heading">
	        <td colspan="2"><?= Loc::getMessage('ALFABANK_PAYMENT_STRING_MODULE_INFO')?>:</td>
	    </tr>
	    <tr>
	        <td width="50%"><?= Loc::getMessage('ALFABANK_PAYMENT_STRING_MODULE_VERSION')?>:</td>
		...
		<tr class="heading">
	        <td colspan="2"><?= Loc::getMessage('ALFABANK_PAYMENT_STRING_PAYMENT_STATUS')?>:</td>
	    </tr>
		<tr>

	        <td width="100%" colspan="2" style="text-align: center;">
	            <select name="RESULT_ORDER_STATUS">
	            	<option value="FALSE"<?= $key == $current_settings['RESULT_ORDER_STATUS'] ? ' selected' : '' ?>><?= Loc::getMessage('ALFABANK_ORDER_STATUS_FALSE')?></option>
	                <?
						$statuses = array();
		...
		<tr class="heading">
	        <td colspan="2"><?= Loc::getMessage('ALFABANK_PAYMENT_CUSTOM_OPTIONS')?>:</td>
	    </tr>
		...
		<tr>
			<td colspan="2">
				<div class="adm-info-message" style="margin-bottom: 5px; max-width: 500px; margin-left: auto; margin-right: auto; display: block; margin-top: 0;">
					<?= Loc::getMessage('ALFABANK_PAYMENT_CUSTOM_OPTIONS_DESCRIPTION');?>
				</div>
			</td>
		</tr>
		...
		<tr class="extra-settings active">
	        <td width="50%" class="adm-detail-content-cell-l">
	        	<?= Loc::getMessage('ALFABANK_PAYMENT_OPTION_PHONE')?>
	        </td>
	    ...
	    <tr class="extra-settings active">
	        <td width="50%" class="adm-detail-content-cell-l">
	        	<?= Loc::getMessage('ALFABANK_PAYMENT_OPTION_EMAIL')?>
	        </td>
	        <td width="50%" class="sberbank-input-top adm-detail-content-cell-r">
	    ...
	    <tr class="extra-settings active">
	        <td width="50%" class="adm-detail-content-cell-l">
	        	<?= Loc::getMessage('ALFABANK_PAYMENT_OPTION_FIO')?>
	        </td>
	        <td width="50%" class="sberbank-input-top adm-detail-content-cell-r">
	    ...
	    <? if ($_REQUEST['server_info'] == '1') { ?>
		    <tr class="heading">
		        <td colspan="2"><?= Loc::getMessage('ALFABANK_PAYMENT_STRING_SERVER_INFO')?>:</td>
		    </tr>
			<?
					$server_info = array();
    ...
    <? $tabControl->Buttons(); ?>
	    ...
	    <input type="button" id="check_server_info" value="<?= GetMessage("ALFABANK_PAYMENT_CHECK_SERVER_INFO") ?>">
	    <script>
	    	 BX.ready(function () {
